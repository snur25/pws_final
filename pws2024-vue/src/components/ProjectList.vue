...
export default {
  components: { ProjectEditor, TasksEditor },
  ...
  methods: {
    ...
    tasksOpen() {
      this.isTasksDialogOpen = true;
      this.editor = false;
    },
    close() {
      this.isTasksDialogOpen = false;
    },
  },
  mounted() {
    fetch(
      personEndpoint +
        "?" +
        new URLSearchParams({ sort: "firstName", order: 1 }).toString()
    ).then((res) =>
      res.json().then((facet) => {
...
<template>
  <v-card variant="outlined">
    ...
    <v-card-text>
      <v-data-table-server
        ...
        @click:row="clickItem"
        itemsPerPageText="# items on page"
        pageText="{0}-{1} of {2}"
        density="compact"
      >
  ...
  <v-dialog v-model="isTasksDialogOpen" width="50%">
    <Tasks @close="close" />
  </v-dialog>
</template>
